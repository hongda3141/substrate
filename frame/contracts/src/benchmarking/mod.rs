...
benchmarks! {
	...
	// We benchmark the call of a contract of size `c` kilobytes. The code is generated to the
	// deploy function so that we don't benchmark the actual execution of instructions. When calling
	// a contract we don't know the size of it. Therefore we initially bill the call with `call(0)`
	// and charge the surcharge from the gas meter right before loading it from storage.
	// The size of the data has no influence on the costs of this extrinsic as long as the contract
	// won't call `seal_input` to copy the data to contract memory. The dummy contract used here
	// does not do this. The costs for the data copy is billed as part of `seal_input`.
	call {
		let c in 0 .. T::Schedule::get().limits.code_len / 1024;
